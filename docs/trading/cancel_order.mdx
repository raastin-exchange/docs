...
این API برای لغو یک سفارش باز (NEW یا PENDING) استفاده می‌شود. پس از لغو موفق، موجودی قفل شده به کیف پول آزاد برمی‌گردد.

**نکته:** فقط سفارش‌هایی که هنوز به طور کامل اجرا نشده‌اند قابل لغو هستند. سفارش‌های با وضعیت FILLED یا CANCELED قابل لغو نیستند.
...
|-----------------|--------|--------|-------------------------------------------------------------|
| id              | String | شرطی   | شناسه سفارش (یکی از id یا client_order_id الزامی است)       |
...
- پس از لغو موفق، موجودی قفل شده **فوراً به کیف پول آزاد** برمی‌گردد
- می‌توانید از `id` یا `client_order_id` برای شناسایی سفارش استفاده کنید
- **سفارش‌های ربات** (strategy orders) بدون دسترسی ویژه قابل لغو نیستند
...
- **Strategy order cancellation not allowed:** سفارش متعلق به ربات است و نیاز به دسترسی ویژه دارد
