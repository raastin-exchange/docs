...
برای ثبت سفارش معامله در بازار راستین از این api استفاده کنید.با درخواست مشاهده وضعیت سفارش می توانید از وضعیت سفارش خود مطلع شوید. برای استفاده از این api لازم است تا در قسمت header توکن خود را ارسال کنید.
...
| price            | String | شرطی        | قیمت سفارش (الزامی برای limit، اختیاری برای market)     |
...
### نکات:
- **قیمت در بازارهای ریالی به ریال است** (مثلاً 3500000000 = 3.5 میلیارد ریال)
- **قیمت در بازارهای تتری به تتر است** (مثلاً 95000.5 = 95 هزار تتر)
- **واحد amount بر اساس ارز معامله‌شونده است** (برای BTCIRT، amount برحسب BTC است)
...
- حداقل ارزش سفارش برای بازارهای ریالی: **100,000 ریال**
- حداقل ارزش سفارش برای بازارهای تتری: **5 تتر**
- برای دریافت `tick_size` و `step_size`، از API `/api/v1/market/symbols` استفاده کنید
